...
workdir = r'y:\Pandas_Work\\'
logfileName = workdir + "logfile.txt"
fname = datadir + 'docList.csv'
logfile = open(logfileName, 'w')
logfile.write("Writing LogFile \n")
def valDate(date): 
#        return str(date)
...
#    print("Start reading:=============================================" + file_path)
    ...
    doclistdf['MRN'] = MRN
    ldcount = len(doclistdf)
    totcount = totcount + ldcount 
    logfile.write(MRN + "=" + str(ldcount) + " Total docs=" + str(totcount) + "\n")    
    return doclistdf
...
outDF.to_excel(workdir + "..\\" + "outdf.xlsx")
...
#    print(MRN)
    ...
    fdl = fdl.append(xldf)
    lenfdla = len(fdl)
    logfile.write("\nlenxl=" + str(lenxl) + " lenfdlb=" + str(lenfdlb) + " lenfdla=" + 
                  str(lenfdla))
    print ("Fdl Rows = " + str(len(fdl)))
...
ChartsWithDocs.to_csv(r'y:\Pandas_Work\chartswithDocs.csv',index=False)
