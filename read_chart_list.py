...
def write_cfg(robot, list):
    if len(list) > 0:
        xlFile =  r'y:\Robot\\' + robot + "_cfg.xlsx"
        print(xlFile)
...
listDir = r'w:\greenway'
patientList = pd.read_csv(listDir + r'\patient_Demographics.csv')
...
rawDocReport = rawDocReport.rename(columns={0: "Date",
                                            2: "Patient",
                                            8: "Type"})
rawDocReport.to_csv(listDir + r"\gw_report_out_2015.csv", index=False)    
'''                                            
'''
...
cleanDocList.to_csv(r'x:\Doc_Reports\Clean_Doc_List.csv')
#noMRN = t[1]   == "NOMRN"
#print(t[noMRN])
'''
