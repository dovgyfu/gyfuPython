...
def walk_doc_lists(directory_path):
    ...
    for root, _, filenames in os.walk(directory_path):
        for filename in filenames:
           ...
           print(filename)
           if (filename.startswith("Medications_")  and filename.endswith(".xls")):
                   print("reading: " + file_path)
                   df1 = pd.read_excel(file_path, header=None,  index=False, skiprows = 1)
                   '''
                   df1 = df1[14:]
                   df1 = df1[[0, 2, 8, 9]]
...
#docDelta = docDelta.rename(columns={'Count_y': 'totCount', 'Count_x': 'haveCount'})                   
    ...
    print(cldf.shape)
    return cldf
repDir = r'y:\MHNI_Data\Medication\\'
meddf = walk_doc_lists(repDir)
meddf = meddf[[0, 1, 2 , 4, 5, 6, 7 ]]
meddf = meddf.rename(columns={0: "medCreateDate",
                              1: "medName",
                              2: "MRN",
                              4: "medSig",
                              5: "medStartDate",
                              6: "medStatus",
                              7: "medEndDate"})
meddf.to_csv(repDir + 'Medications_out.csv', index=False)    
