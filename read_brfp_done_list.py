...
mrnList = []
with open (gyfuDir + 'all_done_bfrp.txt') as fp:
    for line in fp:
        pattern = r'P_(\d+)'
        ...
        if m:
            ...
            print(MRN)
mrnDF = doneDF
mrnDone = mrnDF.drop_duplicates().rename(columns={0: "MRN"})
mrnDone['done_count'] = 1

newMRN = pd.read_csv(gyfuDir + 'mrnList.csv', encoding="ISO-8859-1", header=None, dtype=object)
newMRN = newMRN.drop_duplicates(subset=0)[[0]]
newMRN = newMRN.rename(columns={0: "MRN"})
newMRN['new_count'] = 1
todo = pd.merge(newMRN, mrnDone, how='outer', on='MRN').fillna(0)
todo = todo[todo['done_count'] == 0]
todo.to_csv(gyfuDir + "todoMRN_brfp.csv", index=False)

docList = todo
docList = pd.read_excel(gyfuDir + "sample100.xlsx")
