    ...
    #            print("MRN=" + MRN)
            ...
            for x in docInfo:
                ...
                if key.startswith("Document ID"):
                    ...
                    docfType = value
    #        print(part3)
            repattern = r'DocDate: (.*),DocType: (.*),DocProvider: (.*):,DocModDate: (.*)ModBy: (.*),DocTitle: (.*)-:-'
            m = re.match(repattern, part3)
            if m!= None:
                docDate = m[1]
                docType = m[2]
                docProvider = m[3]
...
#                ModBy = valDate(m[5])
                ...
                if ckm != None:
                    ...
                    print("p3=" + part3)
                    '''
#                    print(robot + " MRN=" + MRN + " " + docDate)
#                    print(docModDate)
                else:
                    ModBy = ck
                    docModDate = valDate(docModDate)
                    
            else:
                print ("ERROR - part3 fails regex")
                print(part3)
            '''
            fields = part3.split(',')
            for x in fields:
...
allLog.to_csv(pandaData + 'all_log.csv', header=False, index=False)
'''
allLog = pd.read_csv(pandaData + 'all_log.csv', header=None)
#allLog.drop('Unnamed: 0', axis=1,inplace=True)
'''
...
doneDocList = pd.read_csv(pandaData + "doneDocList.csv")
doneList = doneDocList
#doneList = doneList[['DocID', 'done_count', 'File', 'MRN', 'DocDate']]
doneList = doneList.drop_duplicates(subset=['DocID'])
doneList = doneList.rename(columns={'MRN': "MRN_done",
                                    'DocType': 'DocType_done'})

mergeList = pd.merge(fullDocLog, doneList, how='outer', on='DocID')

mergeList = mergeList.fillna(0)
fullDocMerge = mergeList
fullDocMerge.to_csv(pandaData + "FullDocMerge.csv",index=False)
mergeList.drop(['CapDate', 'CapTime','Robot'],inplace=True, axis=1)
nofdl = mergeList[mergeList['fdlCount'] == 0]
nofdl['DocID'] = nofdl['DocID'].astype('int')
nofdl['DocID'] = nofdl['DocID'].astype('str')

mergeList = mergeList[mergeList['fdlCount'] == 1]

missingDoc = mergeList[mergeList['done_count'] == 0]
mergeList.to_csv(pandaData + "Full_merge_docs.csv",index=False)

missingDoc = missingDoc[['MRN', 'DocID', 'DocType', 'DocCreateDate']]

missingDoc['pyDate'] = missingDoc.DocCreateDate.map(valDate)
missingDoc = missingDoc.sort_values(['MRN', 'DocType', 'pyDate', 'DocID'],
                                    ascending=[True, True, False, True])
missingDoc.to_excel(pandaData + 'ROBOTX_docSel.xlsx', sheet_name='doc_list', index=False, header=False)
totmissing = len(missingDoc)

grp1 = doneList.groupby(['MRN_done'])
doneMRN = grp1.count()
print('Total Docs Remaining=' + str(totmissing))